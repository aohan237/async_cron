...
class Scheduler:
    def __init__(self, name=None, check_interval: int=5, loop=None):
        """
    ...
    def del_job(self, job_name: str=None):
        if job_name in self.jobs:
            ...
            print(f'{job_name} is in scheduler jobs list')
    ...
    async def check_jobs(self):
        ...
        for name, job in self.jobs.items():
            if job.decide_run():
                now = job.get_now()
                print(f'runing{job.name}',
                      now.humanize(locale='zh_CN'), now)
                job.run()
                if job.remove():
